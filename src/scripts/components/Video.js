export default class Video {
  ...
  constructor(element) {
    ...
    this.videooContainer = this.element.querySelector('.js-video');
    this.poster = this.element.querySelector('.js-poster');

    this.videoId = this.element.dataset.videoId;
    this.autoplay = this.poster ? 1 : 0;
    this.playerReady = false;
    this.control = this.element.parentElement.dataset.hideControl;
    Video.instances.push(this);
    if (this.videoId) {
      Video.loadScript();
    } else {
      console.error('Vous devez spéficier un id');
    }
  }
  ...
  initPlayer(event) {
    ...
    this.player = new YT.Player(this.videooContainer, {
      height: '100%',
      width: '100%',

      videoId: this.videoId,
      playerVars: { rel: 0, autoplay: this.autoplay, control: 1 },
      events: {
        onReady: () => {
